...
export function HostGuests() {
  ...
  const refreshGuests = async () => {
    ...
    try {
      ...
      const mappedGuests = resGuests.data.map((g) => ({
        id: g.bookingId,
        guestId: g.guestId,
      ...
      showMessage({
        type: "error",
        text: "Failed to load guest and property data.",
      });
    } finally {
      setLoading(false);
    }
  ...
  const handleAddGuest = async () => {
    ...
    try {
      ...
      showMessage({
        type: "success",
        text: "Guest booking created successfully!",
      });
    } catch (err) {
      ...
      showMessage({
        ...
        text:
          backendError?.error ||
          "Failed to create booking. Please check details.",
      });
    }
  };
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col">
      <main className="flex-1 p-4 md:p-6 space-y-4 md:space-y-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          ...
          <details
            id="add-guest-details"
            className="bg-white rounded-2xl border border-slate-200 shadow-sm w-full md:w-auto"
          >
