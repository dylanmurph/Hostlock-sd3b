...
export function HostAlerts() {
    ...
    // Filter counts derived from fetched data
    const pendingCount = alerts.filter((a) => a.status === "Pending").length;
    // NOTE: Since TamperAlerts don't have a status field, resolvedCount is currently always 0.
    ...
    useEffect(() => {
        ...
        async function fetchHostAlerts() {
            ...
            setError(null);
            try {
                // AXIOS CALL FOR HOST ALERTS
                const res = await api.get("/host/get/alerts");
                
                // Map the API response structure to the structure expected by the UI component
                const mappedAlerts = mapApiAlertsToUI(res.data);
                
                setAlerts(mappedAlerts);
            } catch (err) {
                console.error("Failed to fetch host alerts:", err.response || err.message);
                setError("Failed to load tamper alerts. Please check the backend service.");
            } finally {
                setLoading(false);
            }
        }
                                    ...
                                    <div className="px-4 py-3 border-b border-slate-100">
                                        <div className="flex items-start justify-between gap-2">
                                            <div className="flex items-start gap-2 md:gap-3 flex-1 min-w-0">
                                                {/* Use the primary icon for Tamper Alerts */}
                                                <IconComponent className="w-5 h-5 md:w-6 md:h-6 text-red-500 mt-0.5 flex-shrink-0" />
                                    ...
                                    <div className="px-4 py-3">
                                        <div className="flex flex-col md:flex-row gap-2">
                                            {alert.status === "Pending" && (
                                                <button 
                                                    onClick={() => alert(`Mark as Resolved for Alert ID: ${alert.id}`)}
                                            ...
                                            <button 
                                                onClick={() => alert(`Notifying host/guest for Alert ID: ${alert.id}`)}
                                                className="inline-flex items-center gap-2 px-3 py-1.5 rounded-lg border border-slate-200 text-xs hover:bg-slate-50">
