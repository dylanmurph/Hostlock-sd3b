...
def get_host_guest_directory():
    ...
    # ---- role check: handle both string + Enum cases safely ----
    role_val = getattr(host, "role", None)
    ...
    # only allow actual hosts / admins through
    if role_str not in ("host", "admin"):
        return jsonify({"error": "Not authorized"}), 403

    # guest filter: handle Enum or string
    guest_query = User.query
    try:
        guest_role_val = UserRole.GUEST
        guest_query = guest_query.filter(User.role == guest_role_val)
    except Exception:
        # if roles are stored as plain strings
        guest_query = guest_query.filter(User.role == "guest")

    guests = guest_query.order_by(User.name.asc()).all()
    ...
    for bnb in bnbs:
        for booking in bnb.bookings:
            ...
            check_in = (
                ...
                else booking.check_in_time
            )
            check_out = (
                booking.check_out_time.replace(tzinfo=timezone.utc)
                if booking.check_out_time.tzinfo is None
                else booking.check_out_time
            )
...
def get_user_profile():
    ...
    return (
        jsonify({"name": user.name, "email": user.email, "photo_path": user.photo_path}),
        200,
    )
...
def create_booking():
    ...
    try:
        ...
        return (
            jsonify({"error": "Incorrect date format. Use YYYY-MM-DDTHH:MM:SS."}),
            400,
        )

    if Booking.query.filter_by(booking_code=booking_code).first():
        return (
            jsonify({"error": f"Booking code '{booking_code}' already in use."}),
            409,
        )
    ...
    try:
        ...
        return (
            jsonify(
                {"error": "Server error while saving booking. Transaction rolled back."}
            ),
            500,
        )
...
def assign_fob_to_booking(booking_id):
    ...
    if booking.bnb.host_id != host_id:
        return jsonify(
            {"error": "Authorization failed. User is not the host of this property."}
        ), 403
...
def add_guest_to_booking(booking_id):
    ...
    if guest.role != "guest":
        return (
            jsonify(
                {
                    "error": f"User '{guest_email}' has role '{guest.role}'. Only 'guest' roles can be added."
                }
            ),
            403,
        )
    ...
    if UserBooking.query.filter_by(user_id=guest.id, booking_id=booking_id).first():
        return (
            jsonify(
                {
                    "error": f"User '{guest_email}' is already a guest for this booking."
                }
            ),
            409,
        )
    ...
    try:
        user_booking = UserBooking(
            user=guest, booking=booking, is_primary_guest=False
        )
        db.session.add(user_booking)
        db.session.commit()
    except Exception:
    ...
    return (
        jsonify(
            {
                "message": "Secondary guest added successfully.",
                "guestId": guest.id,
                "email": guest.email,
            }
        ),
...
def update_booking(booking_id):
    ...
    if new_booking_code == booking.booking_code:
        return (
            jsonify(
                {
                    "message": "Update skipped. Code is unchanged.",
                    "bookingId": booking.id,
                }
            ),
            200,
    ...
    if Booking.query.filter(
        Booking.booking_code == new_booking_code, Booking.id != booking_id
    ).first():
        return (
            jsonify(
                {"error": f"Booking Code '{new_booking_code}' already in use."}
            ),
            409,
        )
...
def delete_booking(booking_id):
    ...
    try:
        ...
        return jsonify(
            {
                "error": "Database error during booking deletion. Transaction rolled back."
            }
        ), 500
...
def remove_guest_from_booking(booking_id):
    ...
    user_booking_link = UserBooking.query.filter_by(
        user_id=guest_id, booking_id=booking_id
    ).first()

    if not user_booking_link:
        return jsonify(
            {"error": f"Guest ID '{guest_id}' not linked to booking '{booking_id}'."}
        ), 404

    if user_booking_link.is_primary_guest:
        return jsonify(
            {"error": "Cannot remove primary guest. Delete booking instead."}
        ), 400
